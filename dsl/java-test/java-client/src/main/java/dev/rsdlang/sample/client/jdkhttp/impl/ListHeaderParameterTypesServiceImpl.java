...
public class ListHeaderParameterTypesServiceImpl implements ListHeaderParameterTypesService {
	...
	public List<String> listStringHeaderParam(List<String> headerValue) {
		...
		$headerParams.put("headerValue", String.join(",",
				headerValue.stream().map($v -> "\"" + ServiceUtils.encodeAsciiString($v) + "\"").toList()));
		var $headers = ServiceUtils.toHeaders($headerParams);
	...
	public String listMultiHeaderParam(List<String> valueA, List<Integer> valueB, List<SimpleRecord.Data> valueC) {
		...
		$headerParams
				.put("valueC",
						String.join(",",
								valueC.stream().map($v -> ServiceUtils.encodeBase64(
										ServiceUtils.ofObject($v, false, this.contentType(), SimpleRecord.Data.class)))
										.toList()));
		var $headers = ServiceUtils.toHeaders($headerParams);
	...
	public List<NilResult> listMultiHeaderParamOpt(List<String> valueA) {
		...
		var $headerParams = new HashMap<String, String>();
		if (valueA != null) {
			$headerParams.put("valueA", String.join(",",
					valueA.stream().map($v -> "\"" + ServiceUtils.encodeAsciiString($v) + "\"").toList()));
		}
		var $headers = ServiceUtils.toHeaders($headerParams);
	...
	public List<SimpleRecord.Data> listRecordHeaderParam(List<SimpleRecord.Data> headerValue) {
		...
		$headerParams
				.put("headerValue",
						String.join(",",
								headerValue.stream().map($v -> ServiceUtils.encodeBase64(
										ServiceUtils.ofObject($v, false, this.contentType(), SimpleRecord.Data.class)))
										.toList()));
		var $headers = ServiceUtils.toHeaders($headerParams);
