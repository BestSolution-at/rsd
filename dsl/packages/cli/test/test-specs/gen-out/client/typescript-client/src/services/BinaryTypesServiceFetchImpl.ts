...
function fnMixed(props: ServiceProps<api.service.ErrorType>): api.service.BinaryTypesService['mixed'] {
	const { baseUrl, fetchAPI = fetch, lifecycleHandlers = {} } = props;
	const { preFetch, onSuccess, onCatch, final } = lifecycleHandlers;
	return async (pathString: string, pathNumber: number, headerString: string, headerNumber: number, headerRecord: api.model.SimpleRecord, queryString: string, queryNumber: number, queryRecord: api.model.SimpleRecord, dataBlob: Blob) => {
		try {
