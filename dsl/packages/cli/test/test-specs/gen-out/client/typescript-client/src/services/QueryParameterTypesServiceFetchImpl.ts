...
import { isRSDBoolean, isRSDDouble, isRSDFloat, isRSDInt, isRSDLocalDate, isRSDLocalDateTime, isRSDLocalTime, isRSDLong, isRSDOffsetDateTime, isRSDShort, isRSDString, isRSDZonedDateTime, type RSDBoolean, type RSDDouble, type RSDFloat, type RSDInt, type RSDLocalDate, type RSDLocalDateTime, type RSDLocalTime, type RSDLong, type RSDOffsetDateTime, type RSDShort, type RSDString, type RSDZonedDateTime } from '../model/Builtins.js';
...
function fnSimpleLongQueryParam(props: ServiceProps<api.service.ErrorType>): api.service.QueryParameterTypesService['simpleLongQueryParam'] {
	...
	return async (queryValue: RSDLong) => {
		try {
			...
			if ($response.status === 200) {
				const $data = await decodeResponse($response, isRSDLong);
