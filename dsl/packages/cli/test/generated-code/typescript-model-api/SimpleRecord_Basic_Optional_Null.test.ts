...
const SimpleNull: SimpleRecord_Basic_Optional_Null = {
	valueBoolean: null,
	valueDouble: null,
	valueFloat: null,
	valueInt: null,
...
const Simple: SimpleRecord_Basic_Optional_Null = {
	valueBoolean: true,
	valueDouble: 1.5,
	valueFloat: 1.5,
	valueInt: 1,
...
describe('isSimpleRecord_Basic_Optional_Null', () => {
	...
	test('additional props', () => {
		expect(isSimpleRecord_Basic_Optional_Null(addFooProperty(Simple))).toBeTruthy();
	});
	test.each(Object.keys(Simple))('missing prop $0', data => {
		const { withOut } = removeProperty(Simple, data);
		expect(isSimpleRecord_Basic_Optional_Null(withOut)).toBeTruthy();
	});
	test.each(Object.keys(Simple))('invalid prop $0', data => {
		expect(isSimpleRecord_Basic_Optional_Null(invalidateProperty(Simple, data))).toBeFalsy();
		expect(isSimpleRecord_Basic_Optional_Null(invalidateProperty(SimpleEmpty, data))).toBeFalsy();
		expect(isSimpleRecord_Basic_Optional_Null(invalidateProperty(SimpleNull, data))).toBeFalsy();
		expect(isSimpleRecord_Basic_Optional_Null(invalidateProperty({}, data))).toBeFalsy();
	});
});
describe('SimpleRecord_Basic_Optional_NullToJSON', () => {
	test('simple', () => {
		...
		expect(SimpleRecord_Basic_Optional_NullToJSON(SimpleEmpty)).not.toBe(SimpleEmpty);

		expect(SimpleRecord_Basic_Optional_NullToJSON({})).toStrictEqual(SimpleEmpty);
		expect(SimpleRecord_Basic_Optional_NullToJSON({})).not.toBe(SimpleEmpty);

		expect(SimpleRecord_Basic_Optional_NullToJSON(SimpleNull)).toStrictEqual(SimpleNull);
