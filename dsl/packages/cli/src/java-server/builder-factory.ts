...
export function generateDTOBuilderFactory(artifactConfig: JavaClientAPIGeneratorConfig): Artifact {
	...
	return {
		...
		content: toString(
			generateCompilationUnit(packageName, importCollector, generateDTOBuilderFactoryContent(packageName, fqn)),
			'\t',
		),
...
function generateDTOBuilderFactoryContent(packageName: string, fqn: (type: string) => string) {
