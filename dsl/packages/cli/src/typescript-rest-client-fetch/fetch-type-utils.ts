...
function ServicePropsContent(fqn: (t: string, typeOnly: boolean) => string) {
	...
	node.indent(l1 => {
		...
		l1.indent(l2 => {
			l2.append('preFetch?: (method: string) => RequestInit | Promise<RequestInit>;', NL);
			l2.append('onSuccess?: (method: string, value: unknown) => void;', NL);
			l2.append(`onError?: (method: string, err: ${fqn('api:../index.ts', false)}.result.RSDError<T>) => void;`, NL);
...
function ifDefinedContent() {
	const node = new CompositeGeneratorNode();
	node.append('export function ifDefined<T>(value: T | undefined, block: (v: T) => void) {', NL);
	node.indent(mBody => {
...
function safeExecuteContent() {
	const node = new CompositeGeneratorNode();
	node.append('export function safeExecute<T>(value: T, block: () => void): T {', NL);
	node.indent(mBody => {
		mBody.append('try {', NL);
		mBody.indent(block => {
