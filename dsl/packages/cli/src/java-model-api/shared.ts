...
import { isMProperty, MResolvedBaseProperty, MResolvedPropery } from '../model.js';
import { computeAPIType, primitiveToObject } from '../java-gen-utils.js';
import { toFirstUpper } from '../util.js';

export function generatePropertyAccessor(property: MResolvedBaseProperty, nativeTypeSubstitues: Record<string, string> | undefined, basePackageName: string, fqn: (type: string) => string) {
	const node = new CompositeGeneratorNode();
	node.append(`public ${computeAPIType(property, nativeTypeSubstitues, basePackageName, fqn)} ${property.name}();`, NL);
	return node;
}

export function generateBuilderPropertyAccessor(property: MResolvedBaseProperty, nativeTypeSubstitues: Record<string, string> | undefined, basePackageName: string, fqn: (type: string) => string, returnType = 'DataBuilder') {
	const node = new CompositeGeneratorNode();
	node.append(`public ${returnType} ${property.name}(${computeAPIType(property, nativeTypeSubstitues, basePackageName, fqn)} ${property.name});`, NL);
	if (isMProperty(property) && !property.array && (property.variant === 'record' || property.variant === 'union')) {
		...
		node.append(NL, `public <T extends ${property.type}.DataBuilder> ${returnType} with${toFirstUpper(property.name)}(Class<T> clazz, ${Function}<T, ${property.type}.Data> block);`, NL);
	}
	return node;
}
...
export function generatePatchBuilderPropertyAccessor(property: MResolvedPropery, nativeTypeSubstitues: Record<string, string> | undefined, basePackageName: string, fqn: (type: string) => string) {
	const node = new CompositeGeneratorNode();
	if (property.variant === 'builtin' || property.variant === 'enum' || property.variant === 'inline-enum' || property.variant === 'scalar') {
		let type = computeAPIType(property, nativeTypeSubstitues, basePackageName, fqn, true);
		...
		if (property.array) {
			node.append(`public PatchBuilder ${property.name}(Patch.${toFirstUpper(property.name)}Change ${property.name});`, NL);
		...
		if (!property.array) {
			...
			node.append(`public <T extends ${baseType}.Builder> PatchBuilder with${toFirstUpper(property.name)}(Class<T> clazz, ${Function}<T, ${baseType}> block);`, NL);
		} else {
			...
			node.append(`public PatchBuilder ${property.name}(${List}<${type}> additions, ${List}<${baseType}.Patch> updates, ${List}<String> removals);`, NL);
			node.append(`public PatchBuilder ${property.name}(List<${type}> elements);`, NL);
		}
	}
	return node;
