...
export function isMBuiltinNumericType(
  value: unknown
): value is NumericBuiltinType {
...
export function isMBuiltinIntegerType(
  value: unknown
): value is IntegerBuiltinType {
...
export function isMRevisionProperty(
  value: unknown
...
export function resolve(model: MRSDModel): MResolvedRSDModel {
  ...
  return {
    ...model,
    elements: model.elements.map((t) =>
      mapToResolved(t, model, solvedMixins, solvedRecords, solvedUnions)
    ),
...
function mapToResolved(
  ...
  if (isMMixinType(t)) {
    ...
    return mapToResolvedRecordType(
      t,
      model,
      solvedMixins,
      solvedRecords,
...
function mapToResolvedOperation(
  o: MOperation,
  model: MRSDModel
): MResolvedOperation {
  return {
    ...
    resolved: {
      errors: o.errors
        .map((e) => model.errors.find((err) => err.name === e))
        .filter(isDefined),
    },
  };
}
