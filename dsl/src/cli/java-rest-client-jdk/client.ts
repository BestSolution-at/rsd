...
export function generateClient(m: MResolvedRSDModel, generatorConfig: ArtifactGenerationConfig, artifactConfig: JavaRestClientJDKGeneratorConfig): Artifact {
    ...
    content.indent( clBody => {
        ...
        clBody.append('}', NL)
        clBody.appendNewLine();
        clBody.append(`public static ${generatorConfig.name}Client create(${URI} baseURI) {`,NL);
        clBody.indent(mBody => {
        ...
        clBody.append(`public <T extends ${BaseDTO}.Builder> T builder(Class<T> clazz) {`, NL)
        clBody.indent( mBody => {
