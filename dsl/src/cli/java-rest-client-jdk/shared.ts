...
import { isMBuiltinType, isMKeyProperty, isMRevisionProperty, MBuiltinType, MKeyProperty, MProperty, MRevisionProperty } from "../model.js"
import { builtinToJavaType, JavaRestClientJDKGeneratorConfig, resolveType } from "../java-gen-utils.js"
import { toType } from "../java-client-api/shared.js"
import { toFirstUpper } from "../util.js"

export function generateProperty(node: IndentNode, property: MKeyProperty | MRevisionProperty | MProperty, artifactConfig: JavaRestClientJDKGeneratorConfig, fqn: (type: string) => string) {
...
export function generateBuilderProperty(node: IndentNode, property: MKeyProperty | MRevisionProperty | MProperty, artifactConfig: JavaRestClientJDKGeneratorConfig, fqn: (type: string) => string, typePrefix?: string) {
    if( isMKeyProperty(property) || isMRevisionProperty(property) ) {
        node.append('@Override', NL)
        node.append(`public ${typePrefix ? `${typePrefix}.`: ''}Builder ${property.name}(${builtinToJavaType(property.type, fqn)} ${property.name}) {`, NL)
        node.indent( methodBody => {
