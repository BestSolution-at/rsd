...
import { generateCompilationUnit, JavaImportsCollector, JavaRestClientJDKGeneratorConfig, resolveObjectType, resolveType, toPath } from "../java-gen-utils.js";
import { MBuiltinType, MOperation, MParameter, MResolvedOperation, MResolvedService, MReturnType } from "../model.js";
import { toType } from "../java-client-api/shared.js";
import { builtinBuilderAccess, builtinBuilderArrayJSONAccess } from "./shared.js";
...
function generateOpertationMethod(
    ...
    if( o.errors.length > 0 ) {
        ...
        node.indent( throwBody => {
            throwBody.append('throws ', fqn(`${artifactConfig.rootPackageName}.${o.errors[0]}Exception`), o.errors.length > 1 ? ',' : '')
    ...
    node.indent( methodBody => {
        ...
        allParameters.filter( p => ! p.nullable ).forEach( (p, idx, arr) => {
        ...
        methodBody.indent( formatted => {
            variables.forEach( (v, idx) => formatted.append(`${v}${idx+1 < variables.length ? ',' : ''}`, NL) )
        })
        ...
        if( hasQueryParams ) {
            ...
            allParameters.filter( p => p.meta?.rest?.source === 'query' ).forEach( (p, idx, arr) => {
                const last = idx + 1 === arr.length;
                methodBody.indent( map => {
                    map.append(`"${p.meta?.rest?.name ?? p.name.toLowerCase()}",ServiceUtils.toQueryString(${p.name})${last ? '' : ','}`,NL)
                });
            })
            methodBody.append(');',NL)
        ...
        const hasHeaderParams = allParameters.find(p => p.meta?.rest?.source === 'header');
        if( hasHeaderParams ) {
            const Map = fqn('java.util.Map');
            methodBody.append(`var $headerParams = ${Map}.of(`, NL)
            allParameters.filter( p => p.meta?.rest?.source === 'header' ).forEach( (p, idx, arr) => {
                const last = idx + 1 === arr.length;
                const Objects = fqn("java.util.Objects");
                methodBody.indent( map => {
                    map.append(`"${p.meta?.rest?.name ?? p.name.toLowerCase()}",${Objects}.toString(${p.name})${last ? '' : ','}`,NL)
                })
            });
            methodBody.append(');',NL)
        ...
        if( method === 'PUT' || method === 'POST' ) {
            ...
            if( bodyParams.length === 0 ) {
                ...
                bodyParams.forEach( p => {
                    if( p.variant === 'record' || p.variant === 'union' ) {
                        if( p.array ) {
                            ...
                            const BaseDTOImpl = fqn(`${artifactConfig.rootPackageName}.jdkhttp.impl.dto.BaseDTOImpl`);
                            methodBody.append(`.add("${p.name}", ((${BaseDTOImpl})${p.name}).data)`,NL);
                        }
                    } else if( p.variant === 'builtin' && isBuiltinType(p.type) ) {
                        if( p.array ) {
                            methodBody.append('$builder = ' + builtinBuilderArrayJSONAccess({ name: p.name, type: p.type })+";", NL)
                        } else {
                            if( p.nullable ) {
                                methodBody.append(`$builder = ${p.name} == null ? $builder.addNull('${p.name}') : ` + builtinBuilderAccess({ name: p.name, type: p.type })+";", NL)
                            } else {
                ...
                const DTOUtils = fqn(`${artifactConfig.rootPackageName}.jdkhttp.impl.dto.DTOUtils`)
                methodBody.append(`var $body = ${BodyPublishers}.ofString(${DTOUtils}.toJsonString($builder.build(),false));`,NL)
            }
            methodBody.appendNewLine()
        }
        ...
        methodBody.indent( tryBlock => {
            tryBlock.append(`var $response = this.client.send($request, ${BodyHandlers}.ofString());`,NL);
            if( o.meta?.rest?.results.length ) {
                o.meta.rest.results.forEach( (r, idx) => {
                    tryBlock.append(`${idx === 0 ? '' : ' else '}if ($response.statusCode() == ${r.statusCode} ) {`, NL)
            ...
            tryBlock.append('throw new IllegalStateException(String.format("Unsupported Http-Status \'%s\':\\n%s", $response.statusCode(), $response.body()));', NL)
...
function handleOkResult(
    ...
    if( o.resultType.variant === 'record' || o.resultType.variant === 'union' ) {
        ...
        if( o.resultType.array ) {
            node.append(`return ServiceUtils.mapObjects($response, ${dtoType}::of);`, NL)
        } else {
            node.append(`return ServiceUtils.mapObject($response, ${dtoType}::of);`, NL)
        }
...
function builtinArrayMap(type: MBuiltinType): string {
    switch( type ) {
        case 'boolean': return 'ServiceUtils.mapBooleans($response)'
        case 'double': return 'ServiceUtils.mapDoubles($response)'
        case 'float': return 'ServiceUtils.mapFloats($response)'
        case 'int': return 'ServiceUtils.mapInts($response)'
        case 'local-date': return 'ServiceUtils.mapLocalDates($response)'
        case 'local-date-time': return 'ServiceUtils.mapLocalDateTimes($response)'
        case 'long': return 'ServiceUtils.mapLongs($response)'
        case 'short': return 'ServiceUtils.mapShorts($response)'
        case 'string': return 'ServiceUtils.mapStrings($response)'
        case 'zoned-date-time': return 'ServiceUtils.mapZonedDateTimes($response)';
    }
}

function builtinMap(type: MBuiltinType): string {
    switch( type ) {
        case 'boolean': return 'ServiceUtils.mapBoolean($response)'
        case 'double': return 'ServiceUtils.mapDouble($response)'
        case 'float': return 'ServiceUtils.mapFloat($response)'
        case 'int': return 'ServiceUtils.mapInt($response)'
        case 'local-date': return 'ServiceUtils.mapLocalDate($response)'
        case 'local-date-time': return 'ServiceUtils.mapLocalDateTime($response)'
        case 'long': return 'ServiceUtils.mapLong($response)'
        case 'short': return 'ServiceUtils.mapShort($response)'
        case 'string': return 'ServiceUtils.mapString($response)'
        case 'zoned-date-time': return 'ServiceUtils.mapZonedDateTime($response)';
    }
}
...
function handleErroResult(
    ...
    node.append(`throw new ${fqn(`${artifactConfig.rootPackageName}.${error}Exception`)}(ServiceUtils.mapString($response), null);`, NL);
}

function computePath(orignalPath: string): { path: string, variables: string[] } {
    const regex = /(\$\{\w+\})/g;
