...
import { generateCompilationUnit, JavaImportsCollector, JavaServerJakartaWSGeneratorConfig, resolveObjectType, resolveType, toPath } from "../java-gen-utils.js";
...
function generateResource(s: MResolvedService, artifactConfig: JavaServerJakartaWSGeneratorConfig): Artifact | undefined {
    ...
    node.indent( cBody => {
        ...
        cBody.append(`public ${s.name}Resource_(${Service} service, ${s.name}ResourceResponseBuilder responseBuilder) {`,NL)
        cBody.indent( mBody => {
        ...
        s.operations.forEach( o => {
            ...
            const multiBody = o.parameters
                .filter( p => p.meta?.rest?.source === undefined ).length > 1;
            const params: string[] = [];
            const serviceParams: string[] = [];

            if( multiBody ) {
                params.push(...o.parameters
                    .filter( p => p.meta?.rest !== undefined )
                    .filter( p => p.meta?.rest?.source !== undefined )
                    .map( p => toParameter(p, artifactConfig, fqn)))
                const DtoType = fqn(`${artifactConfig.rootPackageName}.rest.dto.${s.name}${toFirstUpper(o.name)}DTOImpl`);
                params.push(`${DtoType} dto`)
                serviceParams.push(...o.parameters.map( p => {
                ...
                params.push(...o.parameters.map( p => toParameter(p, artifactConfig, fqn)))
            ...
            cBody.indent( mBody => {
                mBody.append(`var result = service.${o.name}(${serviceParams.join(', ')});`,NL)
                mBody.appendNewLine();
                mBody.append('if (result.isOk()) {',NL)
                mBody.indent( inner => {
                    if( o.resultType ) {
                        if( serviceParams.length === 0 ) {
                            inner.append(`return responseBuilder.${o.name}(result.value()).build();`,NL);
                        } else {
                            inner.append(`return responseBuilder.${o.name}(result.value(),${serviceParams.join(', ')}).build();`,NL);
                        }
                    } else {
                        inner.append(`return responseBuilder.${o.name}(${serviceParams.join(', ')}).build();`,NL);
                    }
                } )
                mBody.append('}',NL)
...
function toParameter(p: MParameter, artifactConfig: JavaServerJakartaWSGeneratorConfig, fqn: (type: string) => string): string {
...
function computeParameterType(p: MParameter, artifactConfig: JavaServerJakartaWSGeneratorConfig, fqn: (type: string) => string): string {
    if( (p.variant === 'record' || p.variant === 'union') && typeof p.type === 'string' ) {
        const Type = fqn(`${artifactConfig.rootPackageName}.rest.dto.${p.type}DTOImpl`);
        if( p.array ) {
        ...
        if( typeof p.type === 'string' ) {
            if( p.array ) {
                const Type = resolveObjectType(p.type, artifactConfig.nativeTypeSubstitues, fqn);
                const List = fqn('java.util.List')
                return `${List}<${Type}>`
            } else {
                return resolveType(p.type, artifactConfig.nativeTypeSubstitues, fqn, p.nullable);
            }
        }
        return 'String';
    }
...
function computeParameterAnnotation(p: MParameter, fqn: (type: string) => string): string {
...
function generateServiceDTO(s: MResolvedService, o: MResolvedOperation, artifactConfig: JavaServerJakartaWSGeneratorConfig): Artifact {
    ...
    node.indent( argNode => {
        ...
        .filter( p => p.meta?.rest?.source === undefined )
        .forEach( (p, idx, arr) => {
            argNode.append(`${toType(p, artifactConfig, fqn, p.nullable)} ${p.name}`)
            if( idx + 1 < arr.length ) {
                argNode.append(',',NL)
            } else {
                argNode.append(') {', NL)
