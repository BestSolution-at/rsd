...
import { isMKeyProperty, isMRevisionProperty, MKeyProperty, MProperty, MRevisionProperty } from "../model.js"
import { builtinToJavaType, JavaServerJakartaWSGeneratorConfig, resolveObjectType, resolveType } from "../java-gen-utils.js"
import { toFirstUpper } from "../util.js"

export function generateConstructorProperty(node: IndentNode, property: MKeyProperty | MRevisionProperty | MProperty, end: string, artifactConfig: JavaServerJakartaWSGeneratorConfig, fqn: (type: string) => string) {
...
export function generateProperty(node: IndentNode, property: MKeyProperty | MRevisionProperty | MProperty, artifactConfig: JavaServerJakartaWSGeneratorConfig, fqn: (type: string) => string) {
    if( isMKeyProperty(property) ) {
...
export function generatePropertyAccess(node: IndentNode, property: MKeyProperty | MRevisionProperty | MProperty, artifactConfig: JavaServerJakartaWSGeneratorConfig, fqn: (type: string) => string) {
    node.appendNewLine();
    if( isMKeyProperty(property) ) {
        node.append(`public ${builtinToJavaType(property.type, fqn)} ${property.name}() {`,NL)
        node.indent( body => {
