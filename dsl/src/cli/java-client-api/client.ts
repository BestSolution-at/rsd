...
export function generateClient(generatorConfig: ArtifactGenerationConfig, artifactConfig: JavaClientAPIGeneratorConfig): Artifact {
    ...
    const clFactoryType = fqn(`${artifactConfig.rootPackageName}.spi.${toFirstUpper(generatorConfig.name)}ClientFactory`);
    const baseDTOType = fqn(`${artifactConfig.rootPackageName}.dto.BaseDTO`);

    const content = new CompositeGeneratorNode();

    content.append(`public interface ${toFirstUpper(generatorConfig.name)}Client {`, NL)
    content.indent( client => {
        client.append(`public static ${toFirstUpper(generatorConfig.name)}Client create(${uriType} baseURL) {`,NL);
        client.indent( body => {
            body.append(`return ${slType}.load(${clFactoryType}.class).iterator().next().create(baseURL);`, NL);
        })
        client.append('}',NL)
        client.append(`public <T extends ${baseDTOType}.Builder> T builder(Class<T> clazz);`, NL)
