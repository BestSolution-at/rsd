...
import { isPatchableRecord_Basic, isPatchableRecord_BasicPatch, type PatchableRecord_Basic, PatchableRecord_BasicFromJSON, type PatchableRecord_BasicPatch, PatchableRecord_BasicToJSON } from './PatchableRecord_Basic.js';
import { checkOptProp, checkProp, createReplaceAddUpdateRemoveGuard, createTypedArrayGuard, isNull, isRecord, isString, isUndefined, type ListMergeAddUpdateRemove, type ListReplace, propMappedListValue, propMappedValue, propValue } from '../_type-utils.js';
import { isMerge, isReplace, Merge, Replace } from '../../../../../check/client/typescript-client/src/_type-utils.js';
...
function isValue_NullPatch(v: unknown): v is $ValuePatch {
	return (isReplace(v) && isPatchableRecord_Basic(v)) || (isMerge(v) && isPatchableRecord_BasicPatch(v));
}

function isValue_OptPatch(v: unknown): v is $ValuePatch {
	return (isReplace(v) && isPatchableRecord_Basic(v)) || (isMerge(v) && isPatchableRecord_BasicPatch(v));
}

function isValue_Opt_NullPatch(v: unknown): v is $ValuePatch {
	return (isReplace(v) && isPatchableRecord_Basic(v)) || (isMerge(v) && isPatchableRecord_BasicPatch(v));
}
